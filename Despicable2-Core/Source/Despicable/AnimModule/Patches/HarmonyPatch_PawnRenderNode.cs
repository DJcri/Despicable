...
public static class HarmonyPatch_PawnRenderNode_AppendRequests
{
    ...
    {
        // Vanilla UI portraits should stay vanilla. The workshop preview is the one portrait render
        // where we *do* want keyframe-driven per-node facing/visibility.
        if (parms.Portrait && !WorkshopRenderContext.Active) return true;
        ...
        if (!WorkshopRenderContext.Active)
        {
            ...
            if (pawn.RaceProps?.Humanlike != true) return true;

            var animator = pawn.TryGetComp<CompExtendedAnimator>();
            if (animator?.hasAnimPlaying != true) return true;
            if (animator.UsesExtendedAnimationFeatures != true) return true;
        }
