...
public struct D2VStack
{
    ...
    public D2VStack(UIContext ctx, Rect bounds)
    {
        _ctx = ctx;
        _bounds = bounds;
        _y = bounds.yMin;
    ...
    [System.Obsolete("Pass ctx is redundant — the D2VStack already holds it. Use NextTextBlock(text, width, font, ...) instead.")]
    ...
    /// <summary>
    /// Allocate AND draw a wrapped text block using the D2VStack's current width.
    /// </summary>
    ...
    [System.Obsolete("Pass ctx is redundant — the D2VStack already holds it. Use NextTextBlock(text, font, ...) instead.")]
    ...
    [System.Obsolete("Pass ctx is redundant — the D2VStack already holds it. Use NextBulletList(items, ...) instead.")]
    ...
    [System.Obsolete("Pass ctx is redundant — the D2VStack already holds it. Use NextSelectorRow(groupId, options, ...) instead.")]
