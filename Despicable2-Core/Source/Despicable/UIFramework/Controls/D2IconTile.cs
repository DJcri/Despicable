...
public static class D2IconTile
{
    ...
    {
        ctx?.Record(rect, UIRectTag.PanelSoft, id ?? "IconTile");
        ...
        D2Widgets.DrawBoxSolid(ctx, rect, fill, (id ?? "IconTile") + "/BG");
        D2Widgets.DrawBox(ctx, rect, hovered ? 2 : 1, (id ?? "IconTile") + "/Border");

        float inset = Mathf.Clamp(iconInset, 0f, Mathf.Min(rect.width, rect.height) * 0.45f);
        Rect iconRect = rect.ContractedBy(inset);
        D2Widgets.DrawTextureFitted(ctx, iconRect, icon, (id ?? "IconTile") + "/Icon");

        if (!tooltip.NullOrEmpty())
        {
            D2Widgets.TooltipHotspot(ctx, rect, (id ?? "IconTile") + "/Tooltip");
