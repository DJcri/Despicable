...
public static class ManualMenuBuilder
{
    ...
    private static readonly MemberInfo disabledMember = ResolveMember(typeof(FloatMenuOption), "Disabled", "disabled");
    private static readonly MemberInfo tooltipMember = ResolveMember(typeof(FloatMenuOption), "tooltip", "Tooltip");
    private static readonly MemberInfo shownItemMember = ResolveMember(typeof(FloatMenuOption), "shownItem", "ShownItemForIcon");
    private static readonly MemberInfo iconThingMember = ResolveMember(typeof(FloatMenuOption), "iconThing", "IconThing");
    private static readonly MemberInfo iconTexMember = ResolveMember(typeof(FloatMenuOption), "iconTex", "IconTex");
    private static readonly MemberInfo iconColorMember = ResolveMember(typeof(FloatMenuOption), "iconColor", "IconColor");
    private static readonly MemberInfo iconJustificationMember = ResolveMember(typeof(FloatMenuOption), "iconJustification", "IconJustification", "horizontalJustification", "HorizontalJustification");
    ...
    private static void ApplyPostBuildState(FloatMenuOption option, ManualMenuOptionSpec spec)
    {
        ...
        if (spec.ShownItemForIcon != null)
            TrySetMemberValue(shownItemMember, option, spec.ShownItemForIcon);

        if (spec.IconThing != null)
            TrySetMemberValue(iconThingMember, option, spec.IconThing);

        if (spec.IconTex != null)
            TrySetMemberValue(iconTexMember, option, spec.IconTex);

        Color? iconColor = ResolveIconColor(spec);
        if (iconColor.HasValue)
            TrySetMemberValue(iconColorMember, option, iconColor.Value);

        TrySetMemberValue(iconJustificationMember, option, spec.IconJustification);

        string tooltip = ResolveTooltip(spec);
        if (!tooltip.NullOrEmpty())
            TrySetMemberValue(tooltipMember, option, tooltip);

        if (spec.IsDisabled)
            TrySetMemberValue(disabledMember, option, true);
    }
    ...
    private static MemberInfo ResolveMember(Type type, params string[] names)
    {
    ...
    private static void TrySetMemberValue(MemberInfo member, object instance, object value)
    {
        ...
        try
        {
            switch (member)
            {
                case FieldInfo field:
                    if (TryConvertValue(value, field.FieldType, out object fieldValue))
                        field.SetValue(instance, fieldValue);
                    break;
                case PropertyInfo property when property.CanWrite:
                    if (TryConvertValue(value, property.PropertyType, out object propertyValue))
        ...
        {
            Log.Warning($"[Despicable2.ManualMenu] Failed to set member '{member.Name}': {ex.Message}");
        }
    }
}