...
def get_auto_definjected_filename(def_type: str) -> str:
    if def_type == 'Despicable.LovinTypeDef':
        return 'D2_NSFW_LovinTypeDef_DefInjected.xml'
...
def prune_orphan_definjected_autogen(root: Path, def_names_by_type: dict[str, set[str]], keep_patterns: list[str]) -> int:
    """Remove orphan nodes from the auto-generated DefInjected file only. Returns removed count."""
    removed_total = 0
    di_root = root / 'Languages' / 'English' / 'DefInjected'
    if not di_root.exists():
        return 0
    for def_type_dir in di_root.iterdir():
        if not def_type_dir.is_dir():
            continue
        auto = def_type_dir / get_auto_definjected_filename(def_type_dir.name)
        if not auto.exists():
            continue
        try:
            tree = ET.parse(auto)
...
def find_orphan_definjected_manual(root: Path, def_names_by_type: dict[str, set[str]], keep_patterns: list[str]) -> list[tuple[Path, str, str]]:
    ...
    for def_type_dir in di_root.iterdir():
        ...
        def_set = def_names_by_type.get(def_type_dir.name, set())
        for f in def_type_dir.glob('*.xml'):
            if f.name == get_auto_definjected_filename(def_type_dir.name):
                continue
            try:
                tree = ET.parse(f)
            except ET.ParseError:
...
def ensure_definjected_autofile(root: Path, def_type: str) -> Path:
    ...
    return d / get_auto_definjected_filename(def_type)
...
def run_localization_checks(root: Path, findings: list[tuple[str, str]], fix_definjected: bool, prune_unused: bool, fail_on_ui_literals: bool = False) -> None:
    ...
    if total_missing:
        if fix_definjected:
            ...
            if added:
                findings.append((WARN, f'Localization: auto-generated {added} missing English DefInjected entry/entries into the auto-generated DefInjected file'))
        else:
    ...
    if fix_definjected:
        ...
        if pruned:
            findings.append((WARN, f'Localization: pruned {pruned} orphan DefInjected node(s) from the auto-generated DefInjected file (use Tools/DefInjectedKeep.txt to keep intentional orphans).'))
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument('--mod-root', default=None, help='Module root to scan (e.g. Despicable2-Core or Despicable2-NSFW). Defaults to the current working directory when it looks like a module root.')
    ...
    parser.add_argument('--fix-definjected', action='store_true', help='Generate missing English DefInjected entries into the auto-generated DefInjected file.')
...
def is_module_root(path: Path) -> bool:
    return (path / 'About').exists() and (path / 'Languages').exists() and (path / 'Source').exists()


def resolve_mod_root(value: str | None) -> Path:
    ...
    if value:
        ...
        if cwd != REPO_ROOT:
            candidates.append(REPO_ROOT / cwd.name)
    ...
    for candidate in candidates:
        ...
        if resolved in seen:
            continue
        seen.add(resolved)
        if is_module_root(resolved):
            return resolved

    details = value if value else str(Path.cwd())
    raise SystemExit(f'Could not resolve a module root from {details!r}. Pass --mod-root Despicable2-Core or --mod-root Despicable2-NSFW, or run from a module root.')


def is_core_module(root: Path) -> bool:
    return (root / 'Source' / 'Despicable' / 'Despicable.csproj').exists() or root.name.lower().endswith('core')
...
def main(argv: list[str] | None = None) -> int:
    ...
    if is_core_module(root):
        check_core_sfw_separation(root, findings)
